...
def forward_pass(w1, b1, w2, b2, input_data):
    a = torch.matmul(input_data, w1) + b1
    hidden_output = torch.sigmoid(a) 
    b = torch.matmul(hidden_output, w2) + b2
    output = torch.softmax(b, dim=1)
...
w1 = torch.normal(mean=0, std=1, size=(input_size, hidden_size), requires_grad=True)
...
b1 = torch.normal(mean=0, std=1, size=(hidden_size,), requires_grad=True)
...
w2 = torch.normal(mean=0, std=1, size=(hidden_size, output_size), requires_grad=True)
...
b2 = torch.normal(mean=0, std=1, size=(output_size,), requires_grad=True)
...
for iteration in range(1, ITERATION+1):
    ...
    with torch.no_grad():
        ...
        train_accuracy = (torch.sum(torch.argmax(train_predictions, dim=1) == torch.argmax(train_label, dim=1)).float() / train_label.shape[0]) * 100
        ...
        validation_accuracy = (torch.sum(torch.argmax(validation_predictions, dim=1) == torch.argmax(validation_label, dim=1)).float() / validation_label.shape[0]) * 100
...
with torch.no_grad():
    ...
    test_accuracy = (torch.sum(torch.argmax(test_predictions, dim=1) == torch.argmax(test_label, dim=1)).float() / test_label.shape[0]) * 100
